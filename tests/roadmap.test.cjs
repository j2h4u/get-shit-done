...
`
  ...
  test('preserves Milestone column in 5-column table', () => {
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'ROADMAP.md'),
      `# Roadmap
...
`
    ...
    const roadmapContent = fs.readFileSync(path.join(tmpDir, '.planning', 'ROADMAP.md'), 'utf-8');
    // Find the table row and verify column order
    const rowMatch = roadmapContent.match(/^\|[^\n]*1\. Test[^\n]*$/m);
    ...
    assert.ok(cells[0].includes('Test'), 'col 0 should be Phase');
    assert.strictEqual(cells[1], 'v1.0', 'col 1 should be Milestone (preserved)');
    assert.strictEqual(cells[2], '1/2', 'col 2 should be Plans');
    assert.ok(cells[3].includes('In Progress'), 'col 3 should be Status');
  });
...
`
    ...
    const roadmapContent = fs.readFileSync(path.join(tmpDir, '.planning', 'ROADMAP.md'), 'utf-8');
    assert.ok(roadmapContent.includes('[x] 01-01'), 'completed plan checkbox should be checked');
    assert.ok(roadmapContent.includes('[ ] 01-02'), 'incomplete plan checkbox should remain unchecked');
  });
